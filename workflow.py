...
try:
    from pyle.util import registry_wrapper2 as registry
except ImportError:
...
DEFAULT_KEYS = {
    'channels': [('timing', ('Preamp',   ['DR Lab Preamp 1', 'A'])),
                 ...
                 ('uwave',  ('Iq',       ['DR Lab DAC 2'])),
                 ('meas',   ('Analog',   ['DR Lab DAC 1', 'A']))],
...
def createRegistryDirectories():
    with labrad.connect() as cxn:
        ...
        wafer = raw_input('Enter wafer name (e.g. w100214A): ')
        die = raw_input('Enter die name (e.g. r5c6): ')

        devices = []
        print 'Enter device names one at a time (e.g. qubit0, res0, coupler0; blank line when done):'
        while True:
            device = raw_input('  Enter device name (<ENTER> if done): ')
            if device == '':
                break
        ...
        session = raw_input('Enter starting session (default = date only): ')
