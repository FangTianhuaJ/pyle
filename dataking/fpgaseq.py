...
def addConfig(p, devices, autotrigger='S3'):
    ...
    for d in devices:
        ...
        if 'settlingRates' in d:
            p.config_settling(d.__name__, d['settlingRates']*GHz, d['settlingAmplitudes'])
    ...
    for i, (t, block) in enumerate(zip(ts, blocks)):
        ...
        for key in groups_ADC.keys():
            ...
            for q in groups_ADC[key]:
                channels = dict(q['channels'])
                if 'readout uwave' in channels:
                    ofs = q['timingLagRRUwave']['ns']
                    t0  = -ofs - prepad
                    rr = rr + q.get('rr', env.NOTHING)
                p.sram_iq_data_fourier((q.__name__, 'readout uwave'), rr(frr, fourier = True), float(t0)*ns)
        ...
        for d in phaseQubits+resonators:
            ...
            if 'uwave' in channels:
                ...
                if idx is not None and xy is not env.NOTHING:
                    xy = xy[idx]
                p.sram_iq_data_fourier((d.__name__, 'uwave'), xy(fxy, fourier=True), float(t0)*ns)
            if 'meas' in channels:
                ofs = d['timingLagWrtMaster']['ns']
                t0 = -ofs - prepad
                # XXX should throw error if t[0] < t0
                ...
                p.sram_analog_data_fourier(d.__name__, z(fz, fourier=True), float(t0)*ns)
