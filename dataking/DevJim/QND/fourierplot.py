...
def fitswap(data, axis=0, return_fit=False):
    ...
    finfo['Isteps'][axis] = 1000.0/n/info['Isteps'][axis]
    finfo['Imin'][axis] = 0.0
    finfo['Imax'][axis] = 500.0/info['Isteps'][axis]
    freqs = np.argmax(fdata, axis=axis) * finfo['Isteps'][axis]
    good = np.argwhere(freqs > 10)[:,0]
    ampl = np.max(fdata, axis=axis)
    zpas = np.linspace(info['Imin'][1-axis], info['Imax'][1-axis], len(freqs))
    freqs = freqs[good]
    ampl = ampl[good]
    zpas = zpas[good]
    avg = avg[good]
    ...
    plt.imshow(data, aspect='auto', origin='lower', cmap=cmap,
               interpolation='nearest',
               extent=(info['Imin'][1] - 0.5 * info['Isteps'][1],
                       info['Imax'][1] + 0.5 * info['Isteps'][1],
                       info['Imin'][0] - 0.5 * info['Isteps'][0],
                       info['Imax'][0] + 0.5 * info['Isteps'][0]), vmin=0)
    
    # plot fourier transform with fit
    plt.subplot(312, sharex=ax)
    plt.imshow(fdata, aspect='auto', origin='lower', cmap=cmap,
               interpolation='nearest',
               extent=(finfo['Imin'][1] - 0.5 * finfo['Isteps'][1],
                       finfo['Imax'][1] + 0.5 * finfo['Isteps'][1],
                       finfo['Imin'][0] - 0.5 * finfo['Isteps'][0],
                       finfo['Imax'][0] + 0.5 * finfo['Isteps'][0]), vmin=0)
