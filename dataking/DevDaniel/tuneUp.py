...
def bringupPulses(s, measure, auto=True):
    ...
    #Now that mpas are know, retune pi amplitudes
    for state in [1,2]:
        if auto:
            mq.pituner(s, measure=measure, state=state, numPulses = 3)
            mq.pituner(s, measure=measure, state=state, numPulses = 3)
        else:
            mq.rabihigh(s, measure=measure, state=state)
            mq.rabihigh(s, measure=measure, state=state)
    #Find tunneling rates
...
def bringupCals(s, measure, dFluxFreq, dZpaFreq, dSwapFreq, dSwapFreq2):
    ...
    mq.swapSpectroscopy(s, measure=measure,
                        ...
                        swapLen = st.r[0:300:5,ns], stats=300)
...
def resonatorSwap(s, measure, busParamName,
                  save=False, noisy=False, collect=False):
    #Tune up |1> swap
    swapTime1, swapAmp1 = mq.swapTuner(s, busParamName, measure=measure, iterations=3,
                                      ampBound=0.025,timeBound=10.0*ns, state=1, noisy=True, save=save)
    #Tune up |2> swap
    swapTime2, swapAmp2 = mq.swapTuner(s, busParamName, measure=measure, iterations=3,
                                      ampBound=0.025,timeBound=10.0*ns, state=2, noisy=True, save=save)
