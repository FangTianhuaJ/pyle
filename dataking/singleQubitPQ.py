...
def squidsteps(sample, measure=0, stats=150,
               ...
               save=True, name='SquidSteps', collect=False, noisy=False,
    ...
    data = sweeps.grid(func, axes, dataset=dataset, save=save, noisy=False)
...
def s_scanning(sample, freq=None, power=None, reset = None, bias=0*V, sb_freq = -50*MHz, measure=0, stats=150,
    ...
    if update:
        ...
        adjust.adjust_s_scanning(Q, phase)
    if collect:
        return data
...
def phase_arc(sample, bias=None, resets=(-2.5*V, 2.5*V), power=None, measure=0, stats=None,
    ...
    if update:
        if rough:
            ...
            adjust.adjust_phase(Q, data)
        else:
            adjust.adjust_phase_arc(Q, data)
    if collect:
        return data

def stepedge(sample, bias=None, stats=300L, measure=0,
             save=True, name='StepEdge', collect=False, noisy=True, update=True):
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    q, Q = qubits[measure], Qubits[measure]
...
def find_step_edge(sample, stats=60, target=0.5, bias_range=None,
                   ...
                   save=False, name='StepEdge Search', collect=False, update=True, noisy=True):
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    q, Q = qubits[measure], Qubits[measure]
...
def scurve(sample, mpa=None, visibility=False, stats=300, measure=0, states=0,
           save=True, name='SCurve', collect=True, noisy=True, update=True):
    ...
    data = sweeps.grid(func, axes, dataset=dataset, save=save, collect=collect, noisy=noisy)
    if update:
        if visibility:
            if max([(states[i+1]-states[i]) for i in range(len(states)-1)])>1:
                print 'Can not update mpa for non-adjacent states.'
            else:
                adjust.adjust_visibility(Q, data, states)
        else:
            adjust.adjust_scurve(Q, data, states)
    if collect:
        return data

def calscurve(sample, stats=30000, measure=0, state=1,
           save=True, name='Cal SCurve', collect=True, noisy=True, update=True):
...
def visibility(sample, mpa=st.r[0:2:0.05], states=1, stats=300, calstats=30000, measure=0,
           save=True, name='Visibility', collect=True, noisy=True, update=True):
...
def spectroscopy(sample, freq=None, stats=300L, measure=0, sb_freq=0*GHz, detunings=None, uwave_amp=None,
                 save=True, name='Spectroscopy', collect=False, noisy=True, update=True):
    sample, qubits, Qubits = util.loadDeviceType(sample, 'phaseQubit', write_access=True)
    q, Q = qubits[measure], Qubits[measure]
    q['readout'] = True
    if freq is None:
...
def spectroscopy_two_state(sample, freq=None, stats=300L, measure=0, sb_freq=0*GHz, detunings=None, uwave_amps=None,
                           save=True, name='Two-state finder spectroscopy', collect=False, noisy=True, update=True):
    sample, qubits, Qubits = util.loadDeviceType(sample, 'phaseQubit', write_access=True)
    q, Q = qubits[measure], Qubits[measure]
...
def find_mpa(sample, stats=60, target=0.05, mpa_range=(-2.0, 2.0), state=1,
             ...
             save=False, name='SCurve Search', collect=True, update=True, noisy=True):
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    q, Q = qubits[measure], Qubits[measure]
    ml.setMultiKeys(q,state)
...
def find_flux_func(sample, freqScan=None, measAmplFunc=None, measure=0,
                   ...
                   save=True, name='Flux func search', collect=False, update=True, noisy=True):
...
def find_zpa_func(sample, freqScan=None, measure=0, spectroscopyAmp = None, state=1,
                  fluxBelow=0.01, fluxAbove=0.01, fluxStep=0.0005, sb_freq=0*GHz, stats=300L,
                  name='ZPA func search', save=True, collect=False, noisy=True, update=True,
                  plot=False):
    """
    if state == 1, find the function of f10 vs zpa,
    if state == 2, find the function of f20/2 vs zpa
    when state is 2, spectroscopyAmp should be large enough to excite the qubit to |2>
    """
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    qubit = qubits[measure]
    if state == 1:
        f_center = qubit['f10']['GHz']
        name += " |1>"
    elif state == 2:
        f_center = (qubit['f10']['GHz'] + qubit['f21']['GHz'])/2.0
        name += " |2>"
    else:
        raise Exception("wrong measured state {}".format(state))

    if spectroscopyAmp is not None:
        qubit['spectroscopyAmp'] = spectroscopyAmp
    if freqScan is None:
        freq = st.nearest(f_center, 0.001)
        dfs = np.logspace(-3, -0.3, 25)
        freqScan = freq + np.hstack(([0], dfs, -dfs))
    elif isinstance(freqScan, tuple) and len(freqScan) == 2:
        freq = st.nearest(f_center, 0.001)
        ...
        freqScan = np.array([f[GHz] for f in freqScan])
    freqScan = freqScan[np.argsort(abs(freqScan-f_center))]
    fluxScan = np.arange(-fluxBelow, fluxAbove, fluxStep)
    fluxScan = fluxScan[np.argsort(abs(fluxScan))]
    fluxPoints = len(fluxScan)
    ...
    if update:
        if state == 1:
            ...
            Qubits[measure]['calZpaFunc2'] = poly
    return get_zpa_func(Qubits[measure], poly)

def get_zpa_func(qubit, p=None, state=1):
    if p is None:
        p = ml.getMultiLevels(qubit, 'calZpaFunc', state=state)
    return lambda f: np.polyval(p, f[GHz]**4)

def freq2zpa(sample, qubitNum, freq, state=1):
    sample, qubits = util.loadQubits(sample)
    qubit = qubits[qubitNum]
    p = ml.getMultiLevels(qubit, 'calZpaFunc', state)
    p = qubit['calZpaFunc']
    zpa = p[1] + p[0]*(freq['GHz']**4)
    return zpa


def zpa2freq(sample, qubitNum, zpa, state=1):
    sample, qubits = util.loadQubits(sample)
    qubit = qubits[qubitNum]
    p = ml.getMultiLevels(qubit, 'calZpaFunc', state)
...
def find_xypa_func(sample, amps=np.linspace(0.1, 1.2, 30), length=st.r[0:200:1, ns],
                 measure=0, stats=600, save=True, noisy=True, collect=True,
                 nfftpoints=4096, update=True, plot=True, forceZero=True, order=2):
    """
    get rabi amp vs rabi frequency,
    freq = a_N * amp^N + a_(N-1) * amp^(N-1) + ... + a_1 * amp + a_0
    and registry key calXYpaFunc = [a_N, a_(N-1), ..., a_1, a_0]

    get rabi frequency vs rabi amp
    amp = b_N * freq^N + b_(N-1)*freq^(N-1) + ... + b_1 * freq + b_0
    and registry key calRabiFreqFunc=[b_N, b_(N-1), ..., b_1, b_0]

    fitting these functions by N-order is due to the experimental results.
    if forceZero is True,
    a_0 and b_0 is zero
    """
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    q, Q = qubits[measure], Qubits[measure]

    data = rabilongHD(sample, length=length, amplitude=amps, detuning=None,
                         measureDelay=None, measure=measure, stats=stats, save=save,
                         collect=True, noisy=noisy, randomize=False, name='xypa func search')

    data = np.asarray(data)
    data = dstools.arrange2Dscan(data)[2]
    rabi_freqs = np.array([fitting.maxFreq(np.array(zip(length,d)), nfftpoints,
                            plot=False)['GHz'] for d in data])
    if forceZero:
        amps = np.array(amps)
        func = np.polyfit(amps, rabi_freqs/amps, order-1)
        func_I = np.polyfit(rabi_freqs, amps/rabi_freqs, order-1)
        func = np.hstack([func, 0.0])
        func_I = np.hstack([func_I, 0.0])
    else:
        func = np.polyfit(amps, rabi_freqs, order)
        func_I = np.polyfit(rabi_freqs, amps, order)
    if update:
...
def get_xypa_func(qubit, p=None):
    if p is None:
        p = qubit['calXYpaFunc']
    func = lambda amp: np.polyval(p, amp)
    return func

def get_rabifreq_func(qubit, p=None):
    if p is None:
        p = qubit['calRabiFreqFunc']
    func = lambda f: np.polyval(p, f)
    return func

def xypa2rabifreq(qubit, amp):
    func = get_xypa_func(qubit)
    freq = func(amp)
    return freq

def rabifreq2xypa(qubit, freq):
    assert isinstance(freq, Value) is True
    freq = freq["GHz"]
    func = get_rabifreq_func(qubit)
    xypa = func(freq)
    return xypa
...
def rabihigh(sample, amplitude=st.r[0.0:1.75:0.05], measureDelay=None, measure=0, state=1, numPulses=1, tBuf=0.0*ns,
             stats=1500L, name='Rabi-pulse height', save=True, collect=False, noisy=True, update=True):
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    q, Q = qubits[measure], Qubits[measure]
    ml.setMultiKeys(q,state)
    name='|'+str(state)+'> '+name
    ...
    def func(server, amp, delay):
        ...
        q['readout'] = True
        return runQubits(server, qubits, stats, probs=[1])
    data = sweeps.grid(func, axes, dataset=dataset, save=save, collect=(collect or update), noisy=noisy)

    if update:
        result = adjust.adjust_rabihigh(Q, data, state=state)
...
def rabilong(sample, length=st.r[0:500:2,ns], amplitude=None, detuning=None, measureDelay=None, measure=0, stats=1500L,
             name='Rabi-pulse length', save=True, collect=False, noisy=True):
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
...
def rabilongHD(sample, length=st.r[0:500:2,ns], amplitude=None, detuning=None, measureDelay=None,
               randomize = True, measure=0, stats=1500L, name='Rabi-pulse length HD', save=True,
...
def freqtuner(sample, iterations=1, tEnd=100*ns, timeRes=1*ns, nfftpoints=4000, stats=1200, df=50*MHz,
    ...
    for i in xrange(iterations):
        data = ramsey(sample, measure=measure, delay=st.r[0:tEnd:timeRes,ns], fringeFreq = df,
                                  stats=stats, name='Ramsey Freq Tuner', save = save, noisy=noisy, state=state,
...
def measureFid(sample, repetition=100, stats=1500, measure=0, level=1,
               save=True, name='measure fidelity', collect=True, update=False, noisy=True):
    sample, qubits, Qubits = util.loadQubits(sample, write_access=True)
    q = qubits[measure]
    Q = Qubits[measure]
    ...
    def func(server, curr):
        ...
        if level == 2:
            ...
            q.xy = eh.boostState(q, t_pi-q['piLen'], state=2)
            q.z = eh.measurePulse2(q, t_meas)
    ...
    result = sweeps.grid(func, axes, dataset=dataset, save=save, collect=collect, noisy=noisy)
    if level==1:
...
def nonlintuner(sample, iterations=1, tEnd=100*ns, timeRes=0.5*ns, nfftpoints=4000, stats=2400,
    ...
    for i in xrange(iterations):
        data = ramseyFilter(sample, st.r[0:tEnd:timeRes,ns], stats=stats,
                            measure=measure, name='Nonlin Tuner', collect=True, noisy=noisy, save=save)
        ts, ps = data.T
...
def amplify_phase_error(sample, phase=np.arange(0,2,0.01)*np.pi, numAmp=(0,1,3,5),
                        measure=0,stats=1200L, name='Amplify Phase Error', save=True,
                        collect=True, noisy=True, alpha=0.5, tBuf=4.0*ns, plot=True):
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
    state = 1
    name = '|%d> %s' %(state, name)

    if not np.iterable(numAmp):
        numAmp = [numAmp]
    axes = [(phase, 'ramsey phase'), (alpha, 'alpha')]
    deps = [('Probability', '|%d> (n=%d)' %(state, n), '') for n in numAmp]
    kw = {'stats': stats, 'alpha': alpha, 'tBuf': tBuf}
    dataset = sweeps.prepDataset(sample, name, axes, deps, measure=measure, kw=kw)

    def ape(qubit, t0, n):
        qubit.xy = env.NOTHING
        for i in range(n):
            qubit.xy += eh.piHalfPulse(qubit, t0, phase=0, alpha=alpha)
            t0 += qubit['piLen'] + tBuf
            qubit.xy += eh.piHalfPulse(qubit, t0, phase=np.pi, alpha=alpha)
            t0 += qubit['piLen'] + tBuf
        return qubit.xy

    def func(server, phi, alpha):
        reqs =[]
        for n in numAmp:
            q.xy = eh.boostState(q, 0, state-1, alpha=alpha)
            t = (state-1)*q.piLen
            q.xy += eh.mix(q, eh.piHalfPulse(q, t, state=1, alpha=alpha))
            t = t+q['piLen']
            q.xy += eh.mix(q, ape(q, t, n))
            t += 2*n*q['piLen'] + 2*n*tBuf
            q.xy += eh.mix(q, eh.piHalfPulse(q, t, phase=phi, alpha=alpha))
            q.z  = eh.measurePulse(q, t+q['piLen']/2, state=state)
            q['readout'] = True
            reqs.append(runQubits(server, qubits, stats, probs=[1]))
        probs = yield FutureList(reqs)
        data = [p[0] for p in probs]
        if noisy: print "phase fraction: ", phi/np.pi
        returnValue(data)
    data = sweeps.grid(func, axes, dataset=dataset, save=save, collect=collect, noisy=noisy)
...
def fidelity_piPulse(sample, alpha=np.linspace(-0.6,0.6,60, endpoint=True), measure=0, stats=1800L, name='piPulse fidelity',
        useAPE=False, numAPE=2, save=True, collect=True, plot=False, noisy=True, tBuf=0*ns):
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]

    axes = [(alpha, 'alpha')]
    kw = {'stats': stats, 'useAPE': useAPE, 'numAPE': numAPE}

    measureFunc = measurement.Octomo(len(sample['config']), measure, tBuf=tBuf)
    dataset = sweeps.prepDataset(sample, name, axes, measure=measureFunc, kw=kw)

    def ape(qubit, t0, n, alpha):
        if not useAPE:
            return env.NOTHING
        qubit.xy = env.NOTHING
        for i in range(n):
            ...
            qubit.xy += eh.piHalfPulse(qubit, t0, phase=np.pi, alpha=alpha)
            t0 += qubit['piLen']
        return qubit.xy * useAPE

    def func(server, a):
        q['alpha'] = a
        q.xy = eh.boostState(q, 0, 1)
        t = q.piLen
        q.xy += eh.mix(q, ape(q, t, numAPE, alpha=a))
        t += numAPE*q.piLen*2*useAPE
        return measureFunc(server, qubits, t-q.piLen/2, stats=stats)

    data = sweeps.grid(func, axes, dataset=dataset, save=save, collect=collect, noisy=noisy)
    rho0 = np.array([[0,0],[0,1]],dtype=np.complex)
    alphas = data[:,0]
    probs = data[:,1:].reshape((-1,6,2))
    rhos = [tomo.qst(diags, 'octomo') for diags in probs]
    Fs =  np.array([fidelity(rho, rho0) for rho in rhos])

    if plot:
        fig = plt.figure()
        plt.subplot(2,1,1)
        octomoKeys = ['I','+X','+Y','-X','-Y','XPi']
        for i, key in enumerate(octomoKeys):
            plt.plot(alphas, probs[:,i,0])
        plt.legend(octomoKeys)
        plt.xlabel('alpha')
        plt.ylabel('Prob|1>')
        plt.subplot(212)
        plt.plot(alphas, Fs.reshape((Fs.size, 1)))
        plt.xlabel('alpha')
        plt.ylabel('Fidelity')

    if collect:
        return np.hstack((data, Fs.reshape((Fs.size, 1))))


def pulseTrajectory(s, fraction=st.r[0.0:1.0:0.01], phase=0.0, alpha=None,
    ...
    return sweeps.grid(func, axes, dataset=dataset, save=save,
                       collect=collect, noisy=noisy, pipesize=1)


def hadamardTrajectory(sample, fraction=st.r[0.0:1.0:0.01], measure=0, stats=1500L, useHD=True,
                       useTomo=True, tBuf=0*ns, name='Hadamard Trajectory', save=True,
                       collect=False, noisy=True):
...
def hadamardInverseTrajectory(sample, fraction=st.r[0.0:2.0:0.01], measure=0, stats=1500L,
                              useHD=True, useTomo=True, tBuf=0*ns, initPulse=None,
                              name='Hadamard Trajectory MQ', save=True, collect=False, noisy=True):
    """
...
def ramseySpec(sample, delay=st.r[0:500:1,ns], swapAmp=np.arange(-0.05,0.05,0.002), phase=0, df=50*MHz,
               measure=0, stats=600L, name='Ramsey spectroscopy 2D', save=True, collect=False, noisy=True):
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
...
def overshootTuner(sample, paramName, measure=0, swapAmp=None, swapTime=None, overshoot=None,
    ...
    if overshoot is None:
        overshoot = qubit['swapOS'+paramName]
    if noisy:
        print 'Original swap overshoot: %f' %qubit['swapOS'+paramName]
    for iteration in range(iterations):
        dataStats=stats*(2**iteration)
        ...
        data = swapSpectroscopy(sample, swapLen=swapTime, swapAmp=swapAmp,
                                overshoot=overshootScan, measure=measure, state=state,
                                collect=True, save=save, stats=dataStats, noisy=noisy,
                                name=name)
        overshoot,probability = fitting.findMinimum(np.array(data),fit=True)
    if noisy:
        print "Best swap overshoot: %f" % overshoot
    if update:
        Qubit['swapOS'+paramName] = overshoot
    return overshoot

def swapPhase(sample, paramName, measure=0, swap_phase=np.linspace(-np.pi, np.pi, 101), swapAmp=None,
              swapTime=None, overshoot=None, state=1, name='Swap Phase Tuner', delay=0.0*ns,
              collect=True, save=True, update=True, stats=1200, noisy=True, correctXtalkZ=False):
    """
    Pi/2 pulse on qubit and then fullswap, and Pi/2 pulse on the qubit,
    By changing the phase of the second pi/2, we will get oscillation,
    find any minimum

    swapAmp, swapTime, overshoot should be a number with or without unit
    delay is the time between two swap
    """
    sample, qubits, _, Qubits = gc.loadQubits(sample, measure, True)
    qubit, Qubit = qubits[measure], Qubits[measure]
    name += " |{}>".format(state) if state>1 else ""
    paramName += str(state) if state >1 else ''
    if swapAmp is None:
        swapAmp = qubit['swapAmp'+paramName]
    if swapTime is None:
        swapTime = qubit['swapTime'+paramName]
    if overshoot is None:
        overshoot = qubit.get('swapOS'+paramName, 0.0)

    axes = [(swap_phase, 'swap phase')]
    indp = [('Probability', '|{}>'.format(state), '')]
    kw = {"stats": stats, "state": state, "delay": delay}
    dataset = sweeps.prepDataset(sample, name, axes=axes, dependents=indp, measure=measure, kw=kw)

    def func(server, curr_phase):
        alg = gc.Algorithm(qubits)
        q = alg.q0
        q['swapPhase'+paramName] = curr_phase
        alg[gates.PiHalfPulse([q], phase=0.0)]
        alg[gates.Swap([q], paramName=paramName, tlen=swapTime, w=None,
                       amp=swapAmp, overshoot=overshoot)]
        alg[gates.Wait([q], waitTime=delay)]
        alg[gates.Swap([q], paramName=paramName, tlen=swapTime, w=None,
                       amp=swapAmp, overshoot=overshoot)]
        alg[gates.PiHalfPulse([q], phase=0.0)]
        alg[gates.MeasurePQ([q])]
        alg.compile(correctXtalkZ)
    ...
    if update:
        phase, prob = data.T
        val = qubit.get('swapPhase'+paramName, 0.0)
        traces = [{'x': phase, 'y': prob, 'args': ('.-', )}]
        params = [{'name': 'swap phase', 'val': float(val), 'range': (min(swap_phase), max(swap_phase)),
                   'axis': 'x', 'color': 'b'}]
        result = adjust.adjust(params, traces)

        if result is not None:
            sele_phase = result['swap phase']
            key = 'swapPhase'+paramName
            print "Old {} : {}".format(key, qubit.get(key, 0))
            ...
            Qubit['swapPhase'+paramName] = fit_val

    if collect:
        return data

def qstFullSwap(sample, paramName, measure=0, repetition=10, swapAmp=None, swapTime=None, axis='+X',
                name='QST - Full Swap', delay=0.0*ns, collect=True, save=True, stats=1200,
                noisy=True, correctXtalkZ=False, plot=True):
    """
    excite qubit with Pi/2 pulse, the axis is given by axis,
    then Full Swap, and finally QST
    @param axis, should be in ["+X", "+Y", "-X", "-Y"]
    """
    ...
    def func(server, i):
        ...
        if axis.upper() == "+X":
            ...
            alg[gates.PiHalfPulse([q0], phase=3*np.pi/2)]
        alg[gates.FullSwap([q0], delay=delay, paramName=paramName)]
        alg.compile()
        t = q0['_t']
    ...
    if plot:
        probs = np.asarray(local_dataset[:, 1:]).reshape(-1, 6, 2)
        rhos = np.array([tomo.qst(diag, 'octomo') for diag in probs])
...
def T1(sample, delay=st.r[-10:1000:2,ns], stats=600L, measure=0,
    ...
    if (plot or update):
        ...
        fitResult = fitting.t1(data,timeRange=(10*ns,delay[-1]))
    if plot:
        data_val = np.array(data)
        fig = dstools.plotDataset1D(data_val, data.indep, data.dep, marker='.', markersize=15)
        ax = fig.get_axes()[0]
        ax.plot(data_val[fitResult.indexList,0],data_val[fitResult.indexList,1],'r.', markersize=15)
        ax.plot(data_val[:,0],fitResult['fitFunc'](data_val[:,0],*fitResult['fitParams']),'k')
        ax.grid()
...
def T1zPulse(sample, delay=st.arangePQ(-10,20,1,ns)+st.arangePQ(20,100,2,ns)+\
                           st.arangePQ(100,500,4,ns)+st.arangePQ(500,1500,10,ns),
             zpa=0, stats=600L, measure=0, state=1,
       name='T1 zPulse', save=True, collect=True, noisy=True):
    """
    T1 with zPulse for |state>.
    default state is 1
    """
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
    ...
    def func(server, delay):
        q.xy = eh.boostState(q, t0=0, state=state)
...
def T1Swap(sample, delay=st.arangePQ(0,1,0.01,'us')+st.arangePQ(1,8,0.1,'us'),
           paraName='C',stats=1200L, measure=0,
       name='T1 Swap', save=True, collect=True, noisy=True, state=1):
    """
    pump the qubit to |state>, swap into another system, delay, swap back to qubit, and measure.
    In this function, T1 of resonator, TLS or something else can be measured.
    """
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
    ...
    if state > 1: paraName += str(state)
    sl = q['swapTime'+paraName]
    sa = q['swapAmp'+paraName]

    def func(server, delay):
        q.xy = eh.boostState(q, 0, state)
        start = (state-0.5)*q['piLen']/2
        q.z = env.rect(start, sl, sa)
        q.z += env.rect(start+sl+delay, sl, sa)
        q.z += eh.measurePulse(q, start+sl+delay+sl)
        q['readout'] = True
        return runQubits(server, qubits, stats, probs=[1])
    data = sweeps.grid(func, axes, dataset=dataset, save=save, noisy=noisy)
    if collect:
...
def resonatorT1(sample, delay=st.arangePQ(0,1,0.01,'us')+st.arangePQ(1,8,0.1,'us'),
           paraName='C',stats=1200L, measure=0,
       name='resonator T1', save=True, collect=True, noisy=True):
    """
    measure resonator T1 by excite the qubit and swap into resonator
    """
    return T1Swap(sample, delay=delay, paraName=paraName,stats=stats,
                  measure=measure, name=name, save=save, collect=collect,
                  noisy=noisy, state=1)
...
def ramsey(sample, measure=0, delay=st.r[0:200:1,ns], phase=0.0, fringeFreq = 50*MHz,
    ...
    def func(server, delay, phase):
        reqs = []
        for tomoPhase in tomoPhasesKeys:
...
def spinEcho(sample, measure=0, delay=st.r[0:1000:10,ns], df=50*MHz,
                   stats=300L, name='Spin Echo', save=True,
                   collect=True, noisy=True, randomize=False, averages=1,
                   tomo=True, plot=True, update=True):
    ...
    if plot or update:
        ...
        if plot:
            ...
            ax.plot(t, result['fitFunc'](t, *result['fitParams']),'k')
            ax.grid()
            plt.title('spinEcho - '+str(dataset['path']))
            fig.show()
        if update:
            Q['calT2Echo']=result['T2']
    return data


def fockScan(sample, n=1, scanLen=0.0*ns, probeFlag=False,stats=1500L, measure=0, delay=0*ns,
       name='Fock state swap length scan', save=False, collect=True, noisy=True):
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
...
def fockScan21(sample, n=1, scanLen=0.0*ns, scanOS=0.0, tuneOS=False, probeFlag=False, stats=1500L, measure=0, delay=0*ns,
       name='Fock state 21 swap length scan', save=False, collect=True, noisy=True):
    sample, qubits = util.loadQubits(sample)
    q = qubits[measure]
...
def fockTuner(sample, n=1, iteration=3, tuneOS=False,stats=1500L, measure=0, delay=0*ns,
       save=False, paramName='', collect=True, noisy=True, update=True):
    sample, qubits, Qubits = util.loadQubits(sample,write_access=True)
    q = qubits[measure]
    Q = Qubits[measure]
