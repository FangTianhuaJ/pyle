...
def x(t):
    return (A*(t/tau)**4)*((1-t/tau)**4)+np.pi/4

def x1(t,dt=1e-4):
    return (x(t+dt) - x(t-dt))/(2*dt)

def x2(t,dt=1e-4):
    return (x1(t+dt) - x1(t-dt))/(2*dt)

def V(t):
    return (x2(t)/(2*np.sqrt(Delta**2/4.0-x1(t)**2)))-(np.sqrt(Delta**2/4.0-x1(t)**2))/np.tan(2*x(t))
...
class SwiphtPulse(NumericalPulse):
    ...
    def timeFunc(self,t):
        return self.amp*V(t-self.t0)* ((t-self.t0)>0) * ((-(t-self.t0)+self.w)>0)
