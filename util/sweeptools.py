...
class IntLogRange(object):
    ...
    def _range(self):
        r, u = self.range, self.unit
        start, stop, N = inUnits(r.start, u), inUnits(r.stop, u), inUnits(r.step, u)
        before_int = np.logspace(np.log10(start), np.log10(stop), N)
        return [int(item) for item in before_int]

    def __iter__(self):
        r = self._range()
        for i in r:
            yield int(i) * self.unit

    def __getitem__(self, i):
        r, u = self.range, self.unit
        start, stop, N = inUnits(r.start, u), inUnits(r.stop, u), inUnits(r.step, u)
        before_int = np.logspace(np.log10(start), np.log10(stop), N)
        after_int=[int(item) for item in before_int]
        try:
            return after_int[i]
        except:
...
class IntLogRangeCreator(object):
    """Create ranges with units using slice notation."""
    def __getitem__(self, range):
        if isinstance(range, tuple):
            range, unit = range
