...
#initially both qubits at same frequency
fq0 = 6.4
fq1 = 6.4
fr0 = 6.2

fc = 6.4 #carrier frequency

S = 0.05 # qubit-resonator coupling

# qubits
q0 = sim.Qubit3(T1=600, T2=400, nonlin=-0.200)
q1 = sim.Qubit3(T1=600, T2=400, nonlin=-0.200)
...
# + Delay(10) +Gaussian_HD(0.096, fwhm=10, len=20, Delta=2*pi*q0.nonlin)
#q0.df = 0
#q0.df = Delay(20) + Square(-0.30, len =10) + Delay(20) + Square(-0.1, len=14)
...
plt.xlabel('time [ns]')
plt.legend(['%d%d' % (i, j) for i in range(q0.n) for j in range(rc.n)])
plt.title('First Pi...then swaps')
