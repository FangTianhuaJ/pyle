...
q2 = sim.Qubit3(T1=500, T2=200, nonlin=-0.200)
...
cc2 = sim.Coupler(q2, rc, s=S)
couplers = [cc0,cc1,cc2]

# the full quantum system
#system = sim.QuantumSystem(qubits, couplers)
system = sim.QuantumSystem([q0,q1,q2,rc],[cc0,cc1,cc2])
...
#q1.df = 0
q1.df = Delay(54) + Square(-0.30, len =10) + Delay(14) + Square(-0.3, len=10)

#Control Qubit
q2.uw = Gaussian_HD(0.096, fwhm=10, len=20, Delta=2*pi*q0.nonlin) 
#q2.df = 0
q2.df = Delay(30) + Square(-0.10, len =14) + Delay(20) + Square(-0.10, len =14) 


# run simulation
psi0 = system.ket('0000')
...
plt.xlabel('time [ns]')
plt.legend(['%d%d%d%d' % (h, i, j, k) for h in range(q0.n) for i in range(q1.n) for j in range(q2.n) for k in range(rc.n)])
